...
#define id_mqtt "ESP-VALV-A"  // definindo identificador para o esp32

// DEFINIÇÃO DE PINOS
#define valvA 5  // definindo a ligação da válvula na porta 5
...
void setup() {
  ...
  digitalWrite(valvA, LOW);  // estado inicial, a válvula A está fechada (LOW)
...
void Task1code(void* pvParameters) {
  ...
  for (;;) {
    const char* msg_status = "ESP32 VALVULA A CONECTADO";
    MQTT.publish("/status/espValvA", msg_status);  // publica o status do ESP32
    //Serial.print("Mensagem publicada no tópico: /status/espValvA");
...
void conectaMQTT() {
  while (!MQTT.connected()) {
    ...
    if (MQTT.connect(id_mqtt, mqttUser, mqttPassword)) {
      ...
      MQTT.subscribe("/comando/valvA");              // subscreve para receber comandos

...
#define id_mqtt "ESP-VALV-C"  // definindo identificador para o esp32

// DEFINIÇÃO DE PINOS
#define valvC 5  // definindo a ligação da válvula na porta 5
...
void setup() {
  ...
  // DECLARANDO OS PINOS
  pinMode(valvC, OUTPUT);    // define a saída para a válvula C como OUTPUT
  digitalWrite(valvC, LOW);  // estado inicial, a válvula C está fechada (LOW)
...
void Task1code(void* pvParameters) {
  ...
  for (;;) {
    const char* msg_status = "ESP32 VALVULA C CONECTADO";
    MQTT.publish("/status/espValvC", msg_status);  // publica o status do ESP32
    Serial.print("Mensagem publicada no tópico: /status/espValvC");
...
void conectaMQTT() {
  while (!MQTT.connected()) {
    ...
    if (MQTT.connect(id_mqtt, mqttUser, mqttPassword)) {
      ...
      MQTT.subscribe("/comando/valvC");              // subscreve para receber comandos
...
void openValve() {
  digitalWrite(valvC, HIGH);  // define o pino da válvula C como HIGH (aberto)
}
// FUNÇÃO FECHAR VÁLVULA
void closeValve() {
  digitalWrite(valvC, LOW);  // define o pino da válvula C como LOW (fechado)
}